...
func initMusaHandles() *musaHandles {
	...
	var mtmusaMgmtPatterns []string
	var musartMgmtPatterns []string

	mtmusaMgmtPatterns = append(mtmusaMgmtPatterns, filepath.Join(LibOllamaPath, "musa_v*", MtmusaMgmtName))
	mtmusaMgmtPatterns = append(mtmusaMgmtPatterns, MtmusaGlobs...)
...
func initVulkanHandles() *vulkanHandles {
	...
	// Short Circuit if we already know which library to use
	if vulkanLibPath != "" && capLibPath != "" {
		vHandles.deviceCount, vHandles.vulkan, _, _ = LoadVulkanMgmt([]string{vulkanLibPath}, []string{capLibPath})
		return vHandles
	}

	libvulkanPaths := FindGPULibs(VulkanMgmtName, VulkanGlobs)
	libcapPaths := FindGPULibs(CapMgmtName, CapGlobs)

	if len(libvulkanPaths) > 0 && len(libcapPaths) > 0 {
		slog.Info("vulkan: load libvulkan and libcap ok")
		vHandles.deviceCount, vHandles.vulkan, vulkanLibPath, capLibPath = LoadVulkanMgmt(libvulkanPaths, libcapPaths)
	} else {
...
func GetGPUInfo() GpuInfoList {
	...
	if !bootstrapped {
		...
		for i := range mHandles.deviceCount {
			if mHandles.mtmusa != nil || mHandles.musart != nil {
				gpuInfo := MusaGPUInfo{
					GpuInfo: GpuInfo{
						Library: "musa",
					},
				...
				// Start with our bundled libraries
				if variant != "" {
					variantPath := filepath.Join(LibOllamaPath, "musa_"+variant)
					if _, err := os.Stat(variantPath); err == nil {
						// Put the variant directory first in the search path to avoid runtime linking to the wrong library
						gpuInfo.DependencyPath = append([]string{variantPath}, gpuInfo.DependencyPath...)
					}
